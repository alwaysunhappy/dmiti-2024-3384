...
@pytest.mark.parametrize("integer, expected", [
    ...
    (Integers(1, [5], False), "5/1"),
    (Integers(1, [5], True), "- 5/1"),
    (Integers(3, [9, 9, 9], False), "999/1"),
])
...
def division_of_fractions(rational_first, rational_second, expected):
...
@pytest.mark.parametrize("coeff, degree, scalar, expected_coeff", [
    (['3/7', '2/5', '3/4'], '2', "1/3", ['1/7', '2/15', '1/4']),
    (['40/1', '-3/5', '31/5', '3/2'], '3', "2/7", ['80/7', '- 6/35', '62/35', '3/7']),
    (['4/6', '-9/4', '5/3', '-37/9'], '3', "-1/9", ['- 2/27', '1/4', '- 5/27', '37/81'])
])
...
@pytest.mark.parametrize("coeff, degree, expected_coeff", [
    (['1/3', '2/5', '3/4'], '2', ['20/1', '24/1', '45/1']),
    (['40/1', '3/4', '31/5', '3/2'], '3', ['800/1', '15/1', '124/1', '30/1']),
])
...
def test_mul_and_div_pol(poly1, poly2):
    assert [str(i) for i in (poly1 * poly2).div_polynom(poly2).coefficients if
            i.numerator.values != [0]] == \
           [str(i) for i in poly1.coefficients]
